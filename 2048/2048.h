...
void xian(void); //显示表格
void jianli(void); //根据全局变量建立表格 四个定点保存到全局变量
int suiji(int a); //随机一个1到a的数
int tianjia(void);  //返回一个随机出来的出现数字链表的一个数
int j_man(int k_s); //返回十字链表中0的数量 向第k_s个0添加一个随机数 为负数不添加
bool s_tian(void); //向十字链表随机位置添加一个随机数 不能消除时返回1 否0
int shhru(int s,int d); //向用户要求输入一个s到d的数
bool fail(void); //当表满时调用 不能消除时返回1 否0
bool paixu(char a); //根据a的值(wasd)排序链表 排序后有变化返回1否0
Shizi *c_kong(Shizi *a,char b,int c,bool d); //c是0时返回a在b(wasd)反方向下的第一个是0结构地址 c是1时返回非零 d是1时跳过搜素a 反之不跳没有找到预期的数返回NULL
